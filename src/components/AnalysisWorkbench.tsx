...
import { BarMeter, ChartPlaceholder } from "@/components/ResearchCharts";
import { analysisCategoryLabels, analysisSkills } from "@/lib/analysisSkills";
import type { Country } from "@/types/Country";
import type { AnalysisSkillCategory } from "@/types/AnalysisSkill";
  ...
  return (
    <section className="mt-8">
      <div className="research-tabs" role="tablist" aria-label="分析方法类别">
        {categories.map((item) => <button key={item} type="button" role="tab" className="research-tab" aria-selected={category === item} onClick={() => { setCategory(item); setResult(null); }}>{analysisCategoryLabels[item]}</button>)}
      </div>
      ...
      {category === "composite_indicators" ? (
        <div className="mt-6 grid gap-6">
          <section className="editorial-panel p-5">
            <div className="grid gap-4 md:grid-cols-3">
              ...
              <label className="text-xs font-semibold text-[var(--muted)]">Analysis skill<select className="field-control mt-2" value={modelId} onChange={(event) => { setModelId(event.target.value as ModelId); setResult(null); }}>{cards.map((item) => <option key={item.model_id} value={item.model_id}>{item.name_zh}</option>)}</select></label>
        ...
        <div className="analysis-category-grid mt-6">{skills.map((skill) => <article key={skill.skill_id} className="editorial-panel p-5"><p className="editorial-kicker">Registry only</p><h2 className="mt-3 text-2xl font-semibold">{skill.name}</h2><p className="mt-3 text-sm leading-7 text-[var(--muted)]">{skill.description}</p><ChartPlaceholder type={category === "network_analysis" ? "network" : category === "bayesian_analysis" ? "distribution" : "coefficient"} note="当前只登记数据要求、参数和诊断，不运行统计估计。" /><details className="advanced-disclosure mt-4"><summary>Registered requirements</summary><p className="mt-3 text-xs text-[var(--muted)]">Required: {skill.required_data.join(" / ")}</p><p className="mt-2 text-xs text-[var(--muted)]">Diagnostics: {skill.diagnostics.join(" / ")}</p></details></article>)}</div>
      )}
    </section>
  );
}
