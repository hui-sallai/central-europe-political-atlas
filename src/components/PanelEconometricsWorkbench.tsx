  ...
  return (
    <div className="mt-6 grid gap-6">
      ...
      {result ? <section className="editorial-panel p-5">
        ...
        <div className="mt-6 overflow-x-auto"><table className="research-data-table w-full min-w-[840px] text-left text-sm"><thead><tr>{["Variable", "Coefficient", result.diagnostics.inference_method === "cluster_country_student_t" ? "Cluster SE" : "HC1 robust SE", "t", result.diagnostics.inference_method === "cluster_country_student_t" ? `p (Student-t, df=${result.diagnostics.degrees_of_freedom})` : "p (asymptotic)", "95% CI"].map((item) => <th key={item} className="px-3 py-3">{item}</th>)}</tr></thead><tbody>{result.coefficients.map((item) => <tr key={item.variable}><td className="px-3 py-3 font-semibold">{panelIndicatorLabels[item.variable] ?? item.variable}</td><td className="metric-number px-3 py-3">{format(item.coefficient)}</td><td className="metric-number px-3 py-3">{format(item.standard_error)}</td><td className="metric-number px-3 py-3">{format(item.t_stat)}</td><td className="metric-number px-3 py-3">{format(item.p_value)}</td><td className="metric-number px-3 py-3">[{format(item.ci_95_low)}, {format(item.ci_95_high)}]</td></tr>)}</tbody></table></div>
        <dl className="mt-6 grid gap-px overflow-hidden border border-[var(--line)] bg-[var(--line)] sm:grid-cols-3 lg:grid-cols-5">{([["N", result.diagnostics.observations], ["Countries", result.diagnostics.countries], ["Years", result.diagnostics.years], ["R²", format(result.diagnostics.r_squared)], ["Within R²", format(result.diagnostics.within_r_squared)], ["Expected rows", result.diagnostics.expected_rows], ["Missing rows", result.diagnostics.missing_rows], ["Coverage", `${(result.diagnostics.sample_coverage * 100).toFixed(1)}%`], ["Clusters", result.diagnostics.clusters ?? "—"], ["Degrees of freedom", result.diagnostics.degrees_of_freedom ?? "—"], ["Inference method", result.diagnostics.inference_method === "cluster_country_student_t" ? "Cluster-robust, Student-t (df = G − 1)" : "HC1 asymptotic"], ["Wild cluster bootstrap", result.bootstrap.status], ["Engine", result.engine_version], ["Dataset", result.dataset_version], ["Platform", result.platform_version]] as Array<[string, string | number]>).map(([label, value]) => <div key={label} className="bg-white p-3"><dt className="text-xs text-[var(--muted)]">{label}</dt><dd className="metric-number mt-1 text-sm font-semibold">{value}</dd></div>)}</dl>
