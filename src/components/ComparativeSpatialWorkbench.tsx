...
function MapPane({
  ...
  return (
    <article className="rounded-2xl border border-[var(--line)] bg-white/75 p-3">
      ...
      {loadState === "error" ? <div className="grid h-[340px] place-items-center rounded-xl bg-amber-50 p-6 text-center text-sm text-amber-950">空间比较待接入：{country.blocker}</div> : null}
      {loadState === "ready" && project ? (
        <>
          ...
          <div className="min-h-12 rounded-xl bg-[var(--surface-muted)] px-3 py-2 text-xs leading-5 text-[var(--muted)]" aria-live="polite">
...
export function ComparativeSpatialWorkbench({
  ...
  const scaleValues = observations.filter((item) => selectedCountries.some((country) => country.country_id === item.country_id) && item.region_indicator_id === effectiveLayer && item.year === effectiveYear).map((item) => item.value);
  ...
  const observationFor = (regionId: string, indicatorId: string) => observations.find((item) => item.region_id === regionId && item.region_indicator_id === indicatorId && item.year === profileYear);
  const rankingRows = observations
  ...
  return (
    <section className="mt-6 card p-4 sm:p-6">
      <div className="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
        <div><p className="eyebrow">Spatial Research Workbench / v0.88</p><h2 className="mt-3 text-2xl font-semibold">区域比较研究工作台</h2><p className="mt-2 max-w-3xl text-sm leading-6 text-[var(--muted)]">选择国家、事实指标与年份，比较同层级区域并追溯来源。这里不生成风险、预测、党派支持率或综合评分。</p></div>
      ...
      <div className="mt-5 grid gap-3 lg:grid-cols-4">
        ...
        <label className="text-xs font-semibold text-[var(--muted)]">图层
          <select value={effectiveLayer} onChange={(event) => setLayer(event.target.value)} className="mt-2 w-full rounded-xl border border-[var(--line)] bg-white px-3 py-2 text-sm text-[var(--foreground)]">{layerOptions.map((item) => <option key={item.layer_id} value={item.layer_id}>{item.name_zh}</option>)}</select>
        </label>
