  ...
  return (
    <section className="mt-6 card p-4 sm:p-6">
      <div className="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
        <div><p className="eyebrow">Spatial Research Workbench / v0.89</p><h2 className="mt-3 text-2xl font-semibold">区域比较研究工作台</h2><p className="mt-2 max-w-3xl text-sm leading-6 text-[var(--muted)]">选择国家、事实指标与年份，比较同层级区域并追溯来源。就业、失业和制造业图层只在统计层级直接匹配时开放，不生成风险、预测或综合评分。</p></div>
      ...
      {currentLayer ? <div className="mt-4 grid gap-2 rounded-2xl border border-[var(--line)] bg-white/65 p-4 text-xs sm:grid-cols-2 lg:grid-cols-4"><div><span className="text-[var(--muted)]">定义</span><p className="mt-1 leading-5">{currentLayer.definition}</p></div><div><span className="text-[var(--muted)]">单位 / 年份</span><p className="mt-1 font-semibold">{currentLayer.unit} / {effectiveYear || "不适用"}</p></div><div><span className="text-[var(--muted)]">空间层级</span><p className="mt-1 font-semibold">{selectedCountries.map((item) => item.admin_level).join(" / ")}</p></div><div><span className="text-[var(--muted)]">比较资格</span><p className="mt-1 leading-5">{mode === "comparison" ? "同层级、同定义、同单位、同年" : "本国内描述性比较"}</p></div></div> : null}
