...
export function DataLayerOverview({ countrySlug, compact = false, title = "数据层总览" }: DataLayerOverviewProps) {
  ...
  const extendedLayer = isV4Country
    ? [{
        title: "V4 扩展观测值",
        value: `${v4Coverage?.present.length ?? 0}/${v4Coverage?.total ?? v4TemplateIndicatorIds.length} 指标`,
        status: "official" as const,
        description: `该国已有财政、外部、投资、能源和产业扩展指标；观测值共 ${extendedObservationCount} 条。`,
        href: "/data",
      }]
    : [];
  ...
  const layers = [
    {
      ...
      description: country ? "政体、议会结构、政府首脑、国家元首和国家摘要已在国家页集中展示。" : "十国国家卡片已建立；深度扩展数据只在对应国家工作台内展示。",
      href: countryHref,
    },
    {
      title: "基础宏观数据",
