...
export function VarWorkbench({ countries }: { countries: Country[] }) {
  ...
  function run() {
    ...
    const specification: VarSpecification = {
      ...
      stationarity_specification_id: deterministicTerms === "constant_month_dummies" ? "adf_constant_seasonal_dummies_mc" : "adf_constant",
          ...
          {tab === "diagnostics" ? (
            <div className="mt-6 grid gap-6">
              <div>
                ...
                <div className="mt-2 overflow-x-auto"><table className="research-data-table w-full min-w-[1320px] text-left text-sm"><thead><tr>{["指标", "变换", "平稳性规格", "ADF 统计量", "1% / 5% / 10% 临界值", "p 值", "滞后", "正式判断", "校准 / 历史参考", "季节持续性", "结构突变状态"].map((header) => <th key={header} className="px-3 py-2">{header}</th>)}</tr></thead>
                  <tbody>{result.stationarity.map((entry) => <tr key={`${entry.indicator}-${entry.transformation}`}><td className="px-3 py-2">{indicatorLabels[entry.indicator] ?? entry.indicator}</td><td className="px-3 py-2">{transformationLabels[entry.transformation]}</td><td className="px-3 py-2">{entry.stationarity_specification_id}</td><td className="metric-number px-3 py-2">{Number.isFinite(entry.adf.statistic) ? entry.adf.statistic.toFixed(3) : "—"}</td><td className="metric-number px-3 py-2">{entry.adf.critical_values["1%"].toFixed(2)} / {entry.adf.critical_values["5%"].toFixed(2)} / {entry.adf.critical_values["10%"].toFixed(2)}</td><td className="metric-number px-3 py-2">{Number.isFinite(entry.adf.p_value) ? entry.adf.p_value?.toFixed(4) : "不提供伪精确值"}</td><td className="metric-number px-3 py-2">{entry.adf.used_lag}</td><td className="px-3 py-2">{entry.formal_decision}</td><td className="px-3 py-2">{entry.adf.calibration ? `MC N=${entry.adf.calibration.calibration_n_lower}–${entry.adf.calibration.calibration_n_upper} · w=${entry.adf.calibration.interpolation_weight.toFixed(3)} · 旧参考 ${entry.legacy_seasonal_adf?.status ?? "—"}` : "MacKinnon-c"}</td><td className="px-3 py-2">lag 12 = {entry.persistence.seasonal_lag_12_autocorrelation?.toFixed(3) ?? "—"}{entry.persistence.seasonal_persistence_warning ? " · 警示" : ""}</td><td className="px-3 py-2">{entry.structural_break_status}</td></tr>)}</tbody></table></div>
                <p className="mt-2 text-xs leading-6 text-[var(--muted)]">v1 使用 ADF + constant；v2 使用 50,000 次/样本节点的 finite-sample calibrated seasonal-dummy ADF（January reference），并保留 v1.43 MacKinnon-c 历史判断。经验 CDF 未发布，因此 p 值保持空；HEGY 为 not_available，Zivot–Andrews 为 registry_only。</p>
              </div>
              <div>
              ...
              <details className="advanced-disclosure"><summary>ACF / PACF 与季节持续性</summary><div className="mt-3 overflow-x-auto"><table className="research-data-table w-full min-w-[820px] text-left text-xs"><thead><tr>{["指标", "lag-1 ACF", "lag-12 ACF", "lag-12 PACF", "近似 95% 带", "季节持续性"].map((header) => <th key={header} className="px-3 py-2">{header}</th>)}</tr></thead><tbody>{result.stationarity.map((entry) => <tr key={entry.indicator}><td className="px-3 py-2">{indicatorLabels[entry.indicator] ?? entry.indicator}</td><td className="metric-number px-3 py-2">{entry.persistence.acf.find((item) => item.lag === 1)?.value.toFixed(3) ?? "—"}</td><td className="metric-number px-3 py-2">{entry.persistence.seasonal_lag_12_autocorrelation?.toFixed(3) ?? "—"}</td><td className="metric-number px-3 py-2">{entry.persistence.pacf.find((item) => item.lag === 12)?.value.toFixed(3) ?? "—"}</td><td className="metric-number px-3 py-2">±{entry.persistence.approximate_significance_band_95.upper.toFixed(3)}</td><td className="px-3 py-2">{entry.persistence.seasonal_persistence_warning ? "seasonal persistence warning" : "未触发描述性警示"}</td></tr>)}</tbody></table></div><p className="mt-2 text-xs text-[var(--muted)]">ACF/PACF 与 ±1.96/√N 近似带仅作描述，不进入 formal stationarity gate；lag-12 自相关明显也不等于确认 seasonal unit root。</p></details>
            </div>
          ) : null}
