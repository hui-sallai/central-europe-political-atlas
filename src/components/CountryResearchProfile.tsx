    ...
    <section className="mt-8">
      ...
      {activeTab === "models" ? (
        <div className="mt-7 grid gap-7 lg:grid-cols-[1fr_0.65fr]">
          ...
          <aside className="editorial-panel p-5"><p className="editorial-kicker">Future Skills</p><h2 className="mt-3 text-2xl font-semibold">统一分析入口</h2><p className="mt-3 text-sm leading-7 text-[var(--muted)]">Panel 与 Network 已正式运行；VAR、Event Study 与 Bayesian 接口已登记，当前版本不运行统计估计。</p><Link href={`/models?country=${country.slug}`} className="mt-5 inline-flex rounded-full bg-[var(--foreground)] px-4 py-2 text-sm font-semibold text-white">进入 Analysis Workbench</Link></aside>
        </div>
      ) : null}
    </section>
  );
