...
import regimes from "@/data/identified-shocks/identification_regime_diagnostics.json";
  ...
  return (
    <section className="editorial-panel p-5">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          <p className="editorial-kicker">Monetary Policy Identification / v1.61</p>
          <h2 className="mt-2 text-2xl font-semibold">货币政策识别</h2>
          <p className="mt-3 max-w-3xl text-sm leading-7 text-[var(--muted)]">EA-MPD 与 EA-EMPD 已完成官方来源审计。Jarociński–Karadi 欧元区输入映射为组合窗口 OIS_3M 与 STOXX50；当前仅开放同号/异号诊断，正式 BVAR 复现和结构分解仍被阻断，因此不标记为已识别冲击，也不开放因果 Local Projections。</p>
        </div>
      ...
      <details className="advanced-disclosure mt-5">
        ...
        <div className="mt-4 grid gap-5 text-sm leading-7 text-[var(--muted)] lg:grid-cols-3">
          ...
          <div><h3 className="font-semibold text-[var(--foreground)]">发布闸门</h3><p className="mt-2">信息效应状态为 {informationEffects.records[0]?.information_effect_handling}。诊断样本中 opposite-sign 为 {regimes.full_sample.diagnostic_counts.policy_dominant}，same-sign 为 {regimes.full_sample.diagnostic_counts.information_dominant}；这些区域不是完整结构识别。只有正式复现、旋转和输出验证通过后，才可能从 external_innovation_proxy 提升为 identified_shock。</p></div>
        </div>
        <p className="mt-4 text-xs text-[var(--muted)]">v1.61 identification decision: {readiness.identification_decision} · JK replication: blocked · Local Projections: {readiness.local_projections_state}</p>
