  ...
  return <main className="page-shell">
    ...
    <Section id="models" label="02 / Analysis" title="透明分析方法">
      ...
      <div className="mt-5 grid gap-5 md:grid-cols-2">
        ...
        <div className="border-l-2 border-[var(--accent)] pl-5 md:col-span-2"><h3 className="font-semibold">Reduced-form VAR / 宏观动态</h3><p className="mt-2 text-sm leading-7 text-[var(--muted)]">v1.42 并列保留两个预注册正式基线：v1 仅含常数项，v2 使用同一组变量、变换、样本政策与 BIC 滞后政策，并增加 11 个月份虚拟变量。月度 HICP 指数和年通胀率均为 NSA；月份虚拟变量只是模型季节控制，不是官方季节调整，也不会改写原始观测。探索性规格只能沿登记链尝试替代变换，并完整记录每次搜索，不能冒充任一正式基线。</p><p className="mt-2 text-sm leading-7 text-[var(--muted)]">每次运行使用共同连续月度样本，报告 AIC、BIC、HQIC，并按确定性项数量正确计算参数门和信息准则。BIC 选出的滞后始终是正式 baseline；完整滞后诊断网格可登记更高阶的“诊断充分备选”，但不会静默替换 baseline。调整 Portmanteau 的 h=12 是主诊断，h=18/24 是较长视野敏感性诊断：6/12 月动态响应要求 h=12 通过，18 月还要求 h=18，24 月还要求 h=24。动态响应是依赖变量排序、没有置信区间的正交化简化式点响应，不是结构冲击或因果效应。</p></div>
      </div>
      <div className="mt-5 border-l-2 border-[var(--accent)] pl-5"><h3 className="font-semibold">启用条件</h3><p className="mt-2 text-sm leading-7 text-[var(--muted)]">来源、年份、单位、状态、更新时间、定义可比性和样本覆盖未通过时不运行。简化式 VAR 至少需要 60 个连续有效月度观测，并须通过已登记变换、ADF 平稳性、共同样本滞后选择、参数数量与稳定性闸门。ADF borderline 最多允许系数估计并显示警告，不开放动态响应；残差门按 6/12/18/24 月视野分别判定，不能用较短视野通过来宣称全部视野可用。KPSS、残差 LM、bootstrap IRF 区间、SVAR、Local Projections、Bayesian VAR、Formal Event Study 与因果政策分析仍未启用。</p></div>
