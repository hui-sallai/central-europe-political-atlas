...
export const dataTableSchemas: DataTableSchema[] = [
  ...
  },
  {
    ...
    fields: [
      ...
      { key: "included_in_derived_comparison", labelZh: "是否进入派生比较", labelEn: "Included in derived comparison", required: true, note: "仅用于最高值、最低值、均值、变化等事实比较，不输出风险指数。" },
      { key: "future_model_eligible", labelZh: "是否可进入未来模型", labelEn: "Future model eligible", required: true, note: "仅表示字段资格；当前平台暂不输出预测或风险模型。" },
      { key: "upward_meaning", labelZh: "数值上升的含义", labelEn: "Meaning of increase", required: true },
      { key: "missing_value_treatment", labelZh: "缺失值处理", labelEn: "Missing value treatment", required: true },
  ...
  },
  {
    ...
    fields: [
      ...
      { key: "country", labelZh: "国家", labelEn: "Country", required: true },
      { key: "region_city", labelZh: "地区/城市", labelEn: "Region / city", required: false },
      { key: "sector", labelZh: "行业", labelEn: "Sector", required: true },
      { key: "chinese_actor", labelZh: "中国主体", labelEn: "Chinese actor", required: false },
      { key: "local_actor", labelZh: "当地主体", labelEn: "Local actor", required: false },
      { key: "amount", labelZh: "金额", labelEn: "Amount", required: false },
      { key: "currency", labelZh: "币种", labelEn: "Currency", required: false, note: "先保留来源原币种，后续再统一换算。" },
      { key: "year", labelZh: "年份", labelEn: "Year", required: false },
      { key: "project_status", labelZh: "状态", labelEn: "Status", required: true },
      { key: "status_timeline", labelZh: "项目状态时间线", labelEn: "Status timeline", required: true },
      { key: "amount_evidence", labelZh: "金额证据/缺失原因", labelEn: "Amount evidence / missing reason", required: true },
      { key: "actor_evidence", labelZh: "主体核验说明", labelEn: "Actor verification notes", required: true },
  ...
  },
  {
    id: "newsEvents",
    title: "政治经济事件表",
    purpose: "把可追溯新闻和官方公告转成结构化事件，并关联现有指标与未来模型候选。",
    usedBy: "事件库、国家页、数据页、事件解释",
    fields: [
      { key: "event_id", labelZh: "事件ID", labelEn: "Event ID", required: true },
      { key: "date", labelZh: "日期", labelEn: "Date", required: true },
      { key: "country", labelZh: "国家", labelEn: "Country", required: true },
      { key: "region_code", labelZh: "区域代码", labelEn: "Region code", required: false },
      { key: "title", labelZh: "标题", labelEn: "Title", required: true },
      { key: "source_name", labelZh: "来源", labelEn: "Source", required: true },
      { key: "topic", labelZh: "主题", labelEn: "Topic", required: true },
      { key: "actor", labelZh: "行为主体", labelEn: "Actor", required: true },
      { key: "event_type", labelZh: "事件类型", labelEn: "Event type", required: true },
      { key: "direction", labelZh: "方向", labelEn: "Direction", required: true, note: "positive、negative、mixed、neutral 或 pending。" },
      ...
      { key: "enters_model", labelZh: "是否进入模型", labelEn: "Enters model", required: true, note: "v0.35 全部为 false。" },
