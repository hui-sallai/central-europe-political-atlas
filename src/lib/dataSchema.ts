...
export const dataTableSchemas: DataTableSchema[] = [
  ...
  },
  {
    ...
    title: "指标字典表",
    purpose: "定义每个指标的口径，保证数据页、模型页和方法论页使用同一套指标解释。",
    usedBy: "数据页、模型页、方法论页",
    fields: [
      ...
      { key: "model_use", labelZh: "模型用途", labelEn: "Model use", required: true, note: "可进入模型、仅展示、禁止进入模型。" },
      { key: "risk_direction", labelZh: "风险方向", labelEn: "Risk direction", required: false, note: "数值上升代表风险上升、下降或中性。" },
  ...
  },
  {
    id: "observations",
    title: "观测值表",
    purpose: "存放每个国家、地区、时间点和指标对应的实际数值，是后续模型计算的核心事实表。",
    usedBy: "数据工作台和后续模型计算",
    fields: [
  ...
  },
  {
    id: "newsEvents",
    title: "新闻事件表",
    purpose: "把新闻卡片转成事件变量，服务新闻页、事件库和模型解释。",
    usedBy: "新闻页、事件库、模型解释",
    fields: [
      ...
      { key: "model_impact", labelZh: "影响模型", labelEn: "Model impact", required: true, note: "进入模型、不进入模型、仅解释。" },
