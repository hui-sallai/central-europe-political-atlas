...
function queryUrl(reporterCode, flowCode, partnerCode) {
...
async function main() {
  const records = [];
  for (const [country, countryName, reporterCode] of countries) {
    const urls = {
      exports_to_china: queryUrl(reporterCode, "X", CHINA_PARTNER_CODE),
      exports_to_world: queryUrl(reporterCode, "X", WORLD_PARTNER_CODE),
      imports_from_china: queryUrl(reporterCode, "M", CHINA_PARTNER_CODE),
      imports_from_world: queryUrl(reporterCode, "M", WORLD_PARTNER_CODE),
    };
    const values = {};
  ...
  fs.writeFileSync(OUTPUT_PATH, `${JSON.stringify({
    schema_version: "china-exposure-trade-inputs-v0.80",
