...
import { LP_ENGINE_VERSION, buildCommonHorizonRows, estimateJointLocalProjection, selectLagAic } from "../../src/lib/localProjectionEngine.mjs";
...
const generatedAt = "2026-09-01"; const shockEnd = "2025-10"; const minimumN = 96;
...
const legacyNormalized = legacy.map((row) => ({ ...row, estimator_ready: false, estimator_validated: false, inference_validated: false, causal_lp_ready: false, blockers: [...new Set((row.blockers ?? []).map((value) => value.replace("Local Projections estimator is not activated in v1.51", "not an identified JK shock under the v1.7 formal LP profile")))] }));
const readinessPayload = { schema_version: "lp-readiness-registry-v1.7", generated_at: generatedAt, method_state: readiness.some((row) => row.causal_lp_ready) ? "active" : "registry_only", readiness_unit: "joint JK shock pair × outcome × country × regime × control profile × maximum horizon", minimum_usable_observations: minimumN, formal_record_count: readiness.length, legacy_record_count: legacyNormalized.length, shock_identification_ready_count: readiness.filter((row) => row.shock_identification_ready).length, sample_ready_count: readiness.filter((row) => row.sample_ready).length, estimator_ready_count: readiness.filter((row) => row.estimator_ready).length, causal_lp_ready_count: readiness.filter((row) => row.causal_lp_ready).length, records: [...readiness, ...legacyNormalized] };
write("lp_reference_manifest.json", { schema_version: "lp-reference-manifest-v1.7", generated_at: generatedAt, repository: "jm4474/Lag-augmented_LocalProjections", repository_url: "https://github.com/jm4474/Lag-augmented_LocalProjections", pinned_commit: "02e8e65396f2c06d2c87879bbc5c2162863b907f", commit_date: "2022-12-21T00:46:04Z", latest_commit_at_retrieval: "02e8e65396f2c06d2c87879bbc5c2162863b907f", newer_commit_detected: false, license: "MIT", files_sha256: { "README.md": "86b6fc57233d1c79a626b99af3dc369d2bb75e50ffcfbc8df74df56e77987abe", "LICENSE": "fa13b6927d93a253b28506e0692e17dad33c3f3035ef2fea9a6f04a45ed0b6b5", "functions/ir_estim.m": "50235300c3b84ad39d6577900eae39be52498d3daeed3ab92490fa6bbd3bf753", "functions/lp.m": "e4afccf8dd5949a6e22224291ffec85c73fdb46e9abe0481b6da9546d923509a", "functions/linreg.m": "870b3322efc5858491af3ba7e9752d7218009242cccc9178be3ebc74b93dba32", "functions/ewc.m": "c9608921da0ea8a32565230dc2762bfb2ad27a1b7abb774044b3fce162ce4da7" }, implementation_audit: "ir_estim.m passes p-1+lag_aug lags into lp.m; with lag_aug=true, the base order p therefore yields contemporaneous regressors plus p full lags. Primary delta-method inference uses EHW with HC1 finite-sample adjustment and normal critical values; no HAC required." });
write("lp_specification_registry.json", { schema_version: "lp-specification-registry-v1.7", generated_at: generatedAt, engine_version: LP_ENGINE_VERSION, estimation_unit: "single country × outcome × joint JK shock pair × specification", joint_shocks: true, panel_lp: false, state_dependent_lp: false, baseline_equation: "cumulative/level response at h on contemporaneous MP_t and CBI_t, p lag-augmented lags of outcome/MP/CBI, deterministics", frozen_shock_construction: true });
write("lp_outcome_specification_registry.json", { schema_version: "lp-outcome-specification-registry-v1.7", generated_at: generatedAt, records: outcomes.map((row) => Object.fromEntries(Object.entries(row).filter(([key]) => key !== "transform"))) });
write("lp_sample_policy_registry.json", { schema_version: "lp-sample-policy-registry-v1.7", generated_at: generatedAt, shock_end: shockEnd, candidate_horizons: [24, 18, 12, 6], common_horizon_sample: true, minimum_usable_observations: minimumN, no_event_shock_value: 0, meeting_month_selection: false, horizon_fallback_must_be_reported: true });
write("lp_lag_policy_registry.json", { schema_version: "lp-lag-policy-registry-v1.7", generated_at: generatedAt, baseline: { candidate_lags: [1,2,3,4,5,6], criterion: "AIC", selection_frequency: "once per country × outcome × regime on a common pre-estimation sample", horizon_specific_selection: false, lag_augmentation: "add one full lag relative to non-augmented LP; reported augmented order p+1" }, sensitivity: [{ profile: "fixed_p2", lag: 2 }, { profile: "fixed_p6", lag: 6 }] , selections: lagSelections });
write("lp_control_profile_registry.json", { schema_version: "lp-control-profile-registry-v1.7", generated_at: generatedAt, records: [{ profile_id: "formal_baseline", controls: ["constant", "MP_t", "CBI_t", "p lags of outcome, MP and CBI"], hicp_addition: "11 month-of-year dummies", contemporaneous_mediators_excluded: ["domestic policy rate", "FX", "bond yield", "energy price"] }, { profile_id: "predetermined_sensitivity", state: "registry_only", lagged_controls: ["Brent", "European gas", "domestic policy rate for non-euro countries"] }] });
write("lp_readiness_registry.json", readinessPayload); write("lp_model_registry.json", { schema_version: "lp-model-registry-v1.7", generated_at: generatedAt, record_count: models.length, records: models }); write("lp_results.json", { schema_version: "lp-results-v1.7", generated_at: generatedAt, model_count: results.length, horizon_record_count: results.reduce((s, row) => s + row.horizons.length, 0), records: results });
...
skills.schema_version = "analysis-skill-registry-v1.7"; skills.generated_at = generatedAt;
skills.records = skills.records.map((row) => row.skill_id === "local_projections" ? { ...row, state: "active", gate: "identified joint JK shocks + applicable country/outcome + common-horizon N >= 96 + validated lag-augmented estimator/inference", readiness_reference: "local-projections/lp_readiness_registry.json", note: "Single-country joint MP/CBI Local Projections only; pointwise intervals; no panel LP or state dependence." } : row.skill_id === "monetary_policy_identification" ? { ...row, note: "Median decomposition remains author-reference validated; v1.7 Local Projections use only the two frozen identified JK shock series." } : row);
fs.writeFileSync(skillsPath, `${JSON.stringify(skills, null, 2)}\n`);
console.log(`v1.7 LP build: formal=${readiness.length}; causal_ready=${readinessPayload.causal_lp_ready_count}; models=${models.length}; horizons=${results.reduce((s,r)=>s+r.horizons.length,0)}.`);
